...
def parse_args():
    ...
    parser.add_argument("--save-dir",default=os.getcwd(),
        help="Directory to save the results. Default is current working directory.")
...
def fuse(samples: int, base_dir: str = os.getcwd(), save_dir: str = os.getcwd(), gt_floorplan_path: str ="", complete: bool = False):
    ...
    os.makedirs(os.path.join(save_dir, sample_dir, USED_DIRNAME), exist_ok=True)
    os.makedirs(os.path.join(save_dir, sample_dir, IGNORED_DIRNAME), exist_ok=True)
    ...
    for image_path in image_paths:
        ...
        if abs(image.shape[0]-max_height) >= MAX_HEIGHT_ERROR or abs(image.shape[1]-max_width) >= MAX_WIDTH_ERROR:
            ...
            cv2.imwrite(os.path.join(save_dir, sample_dir, IGNORED_DIRNAME, os.path.basename(image_path)), image)
            continue
        elif to_initialize:
        ...
        cv2.imwrite(os.path.join(save_dir, sample_dir, USED_DIRNAME, os.path.basename(image_path)), image)
    ...
    cv2.imwrite(os.path.join(save_dir, sample_dir, "addition.png"), addition)
    ...
    with open(os.path.join(save_dir, sample_dir, "costs.txt"), "w") as costs_file:
        for h in range(max_height):
            for w in range(max_width):
                costs_file.write(f"{w} {h} {costs[h, w]}\n")

    cv2.imwrite(os.path.join(save_dir, sample_dir, "threshold.png"), addition_thresholded)
    cv2.imwrite(os.path.join(save_dir, sample_dir, "otsu.png"), otsu)
    cv2.imwrite(os.path.join(save_dir, sample_dir, "tri.png"), tri)
    cv2.imwrite(os.path.join(save_dir, sample_dir, "floorplan.png"), floorplan)
    cv2.imwrite(os.path.join(save_dir, sample_dir, "added_map.png"), addition)
    ...
    cv2.imwrite(os.path.join(save_dir, sample_dir, "summary.png"), summary)
...
def main():
    ...
    if not sample_test:
        fuse(base_dir=base_dir, save_dir=save_dir, gt_floorplan_path=gt_floorplan_path, samples=max_size, complete=True)
    else:
        # Note: real sample size <= given sample size because some maps may be discarded
        for sample_size in range(step, max_size+1, step):
            fuse(base_dir=base_dir, save_dir=save_dir, gt_floorplan_path=gt_floorplan_path, samples=sample_size)
        if (max_size + 1) % step != 0:
            fuse(base_dir=base_dir, save_dir=save_dir, gt_floorplan_path=gt_floorplan_path, samples=max_size)
